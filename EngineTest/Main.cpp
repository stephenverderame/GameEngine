...
//	cam.setTarget(glm::vec3(0));
	...
	engine.view(cam);
	Scene scene;
	std::vector<glm::mat4> guys = { Engine::makeTransformation(glm::vec3(0)), Engine::makeTransformation(glm::vec3(1)), Engine::makeTransformation(glm::vec3(-1)) };
	Model guy("C:\\Users\\stephen\\Downloads\\crysis-nano-suit-2\\everything\\scene.fbx", guys);
	...
	dirLight d(glm::vec3(1, 5, 0), glm::vec3(0.9), 0.3, 0.01);
	scene.addLight(&p);
	scene.addLight(&s);
	scene.addLight(&d);
